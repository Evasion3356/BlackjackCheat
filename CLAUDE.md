...
(deploy step off), zips it as `BlackjackCheat-X.Y.zip` (the `.asi` plus the default `BlackjackCheat.ini`), publishes a GitHub release named
"BlackjackCheat X.Y.Z" with that changelog entry as its notes and the zip and `.asi`
...
`bin\<Configuration>\BlackjackCheat.ini` is generated on every Debug/Release build
